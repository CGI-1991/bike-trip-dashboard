...
export function getActiveTripId(storage: Storage = window.localStorage): TripId | null {
  try {
...
export function setActiveTripId(tripId: TripId, storage: Storage = window.localStorage): boolean {
  if (!isNonEmptyString(tripId)) return false
  try {
...
export function clearActiveTripId(storage: Storage = window.localStorage): boolean {
  try {
