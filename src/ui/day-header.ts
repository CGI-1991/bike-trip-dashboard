...
export function renderDayHeader(container: HTMLElement, timeline: TripDayTimeline | null, weatherLevel: 'green' | 'orange' | 'red' | 'unknown' = 'unknown'): void {
  ...
  container.innerHTML = `<div class="day-identity"><p class="eyebrow">${day.id} · <time datetime="${date}">${formattedDate}</time></p><h2>${day.startName} → ${day.endName}</h2></div><dl class="day-header__metrics"><div><dt>Distance</dt><dd>${summary.distanceKm.toFixed(1)} km</dd></div><div><dt>D+</dt><dd>${integers.format(summary.elevationGainM)} m</dd></div><div><dt>Départ</dt><dd>${timeline.startTime}</dd></div><div><dt>ETA</dt><dd>${clock(timeline.arrivalTime.clockMinutes)}</dd></div></dl>${risk}`
}
