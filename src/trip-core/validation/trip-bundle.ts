...
export function validateTripBundle(value: unknown): ValidationResult<TripBundle> {
  ...
  sourceFiles.forEach((file, index) => {
    ...
    if (file.sha256 !== null && !isNonEmptyString(file.sha256)) {
      issues.push(issue(`${path}.sha256`, 'invalid-value', 'sha256 doit être une chaîne non vide ou null.'))
    }
    ...
    if (!isStringArray(file.parsingErrors)) issues.push(issue(`${path}.parsingErrors`, 'invalid-type', 'parsingErrors doit être un tableau de chaînes.'))
  })
  ...
  routes.forEach((route, index) => {
    ...
    if (!Array.isArray(route.segments)) {
      ...
      route.segments.forEach((segment: unknown, segmentIndex: number) => {
        ...
        if (!isNonNegativeInteger(segment.index)) issues.push(issue(`${segmentPath}.index`, 'invalid-value', 'index de segment invalide.'))
      ...
      if (!isPlainObject(route.geometry)) {
        ...
        for (const key of ['full', 'simplified'] as const) {
          ...
          points.forEach((point: unknown, pointIndex: number) => {
            ...
            if (!isPlainObject(point) || !isFiniteNumber(point.latitude) || !isFiniteNumber(point.longitude)) {
              issues.push(issue(pointPath, 'invalid-value', 'Point de géométrie invalide.'))
              return
            }
    ...
    if (!isStringArray(route.parsingErrors)) issues.push(issue(`${path}.parsingErrors`, 'invalid-type', 'parsingErrors doit être un tableau de chaînes.'))
  ...
  collectIds(practicalPlaces, 'id', 'practicalPlaces', issues)
  ...
  days.forEach((day, index) => {
    ...
    if (calendarStartDate === null && day.date !== null && day.date !== undefined) {
      issues.push(issue(`${path}.date`, 'inconsistent-calendar', 'Une journée datée exige un calendrier défini (calendar.startDate).'))
    }
    if (
      calendarStartDate !== null &&
      isIsoDate(day.date) &&
      ((day.date as string) < calendarStartDate || (calendarEndDate !== null && (day.date as string) > calendarEndDate))
    ) {
      issues.push(issue(`${path}.date`, 'date-outside-calendar', 'date en dehors de la plage calendar.startDate/endDate.'))
    }
    if (!isOneOf(day.type, TRIP_DAY_TYPES)) {
      ...
      if (day.type === 'ride') {
        ...
        if (day.stageId !== null) issues.push(issue(`${path}.stageId`, 'unexpected-value', 'Une journée off ne référence aucune étape.'))
      } else if (day.stageId !== null && !isNonEmptyString(day.stageId)) {
  ...
  stages.forEach((stage, index) => {
    ...
    for (const field of ['distanceKm', 'elevationGainM', 'elevationLossM', 'movingDurationSeconds', 'pauseDurationSeconds', 'totalDurationSeconds', 'estimatedAverageSpeedKph'] as const) {
    ...
    for (const [field, knownIds] of [
      ...
      fieldValue.forEach((referencedId, referenceIndex) => {
        if (!knownIds.has(referencedId)) {
          issues.push(issue(`${path}.${field}[${referenceIndex}]`, 'unknown-reference', `${field} inconnu : ${referencedId}.`))
        }
      })
    }
  ...
  collectIds(weather, 'id', 'weather', issues)
  weather.forEach((record, index) => {
    ...
    if (!isIsoDate(record.forDate)) issues.push(issue(`${path}.forDate`, 'invalid-date', 'forDate invalide.'))
    ...
    for (const field of ['temperatureMinC', 'temperatureMaxC', 'windSpeedKph'] as const) {
      if (record[field] !== null && !isFiniteNumber(record[field])) issues.push(issue(`${path}.${field}`, 'invalid-value', `${field} invalide.`))
    }
  ...
  if (!isPlainObject(settings)) {
    ...
    settingsStages.forEach((entry, index) => {
      ...
      if (!isNonEmptyString(entry.stageId)) {
        ...
        if (!stageIds.has(entry.stageId)) issues.push(issue(`${path}.stageId`, 'unknown-reference', `stageId inconnu : ${entry.stageId}.`))
      ...
      if (!Array.isArray(entry.pauses)) {
        ...
        entry.pauses.forEach((pause: unknown, pauseIndex: number) => {
          ...
          if (pause.routePointId !== null && !isNonEmptyString(pause.routePointId)) issues.push(issue(`${pausePath}.routePointId`, 'invalid-value', 'routePointId doit être une chaîne non vide ou null.'))
          if (!isNonNegativeNumber(pause.durationSeconds)) issues.push(issue(`${pausePath}.durationSeconds`, 'invalid-value', 'durationSeconds doit être ≥ 0.'))
          if (!isNonNegativeInteger(pause.order)) issues.push(issue(`${pausePath}.order`, 'invalid-value', 'order doit être un entier ≥ 0.'))
  ...
  overrides.forEach((override, index) => {
    ...
    if (!isOneOf(override.targetType, OVERRIDE_TARGET_TYPES)) issues.push(issue(`${path}.targetType`, 'invalid-enum', 'targetType invalide.'))
    if (!isNonEmptyString(override.targetId)) issues.push(issue(`${path}.targetId`, 'missing-required', 'targetId est requis.'))
