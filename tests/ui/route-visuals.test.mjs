...
test('the expanded map opens before its size and route bounds are fitted with modal padding', () => {
  ...
  const expandedFitIndex = source.indexOf('invalidateBeforeInitialFit: true', animationFrameIndex)
  const invalidationIndex = source.indexOf('map.invalidateSize()')
  const fitBoundsIndex = source.indexOf('map.fitBounds(', invalidationIndex)
  assert.ok(showModalIndex > 0)
  assert.ok(animationFrameIndex > showModalIndex)
  assert.ok(expandedFitIndex > animationFrameIndex)
  assert.ok(invalidationIndex > 0 && fitBoundsIndex > invalidationIndex)
  assert.match(source, /fitPadding: \[36, 36\]/)
  assert.match(source, /maxInitialZoom: 13/)
