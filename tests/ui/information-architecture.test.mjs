...
test('the settings editor uses shrinkable grid children so the native time input stays inside a 320 px dialog', () => {
  ...
  assert.match(css, /\.pause-editor \{[^}]*max-inline-size: calc\(100vw - 20px\)/s)
  assert.match(css, /\.pause-editor form \{[^}]*min-inline-size: 0;[^}]*max-inline-size: 100%/s)
  assert.match(css, /\.pause-editor fieldset, \.day-settings-fields,[^{]+\{[^}]*min-inline-size: 0;[^}]*max-inline-size: 100%/s)
  assert.match(css, /\.day-settings-fields \{[^}]*grid-template-columns: minmax\(0, 1fr\)/s)
  assert.match(css, /\.day-settings-fields input \{[^}]*inline-size: 100%;[^}]*min-inline-size: 0;[^}]*max-inline-size: 100%/s)
  assert.match(css, /\.field__time-control \{[^}]*display: grid;[^}]*minmax\(0, 1fr\)/s)
  assert.match(css, /\.field__time-control input\[type='time'\] \{[^}]*inline-size: 100%;[^}]*min-inline-size: 0;[^}]*max-inline-size: 100%/s)
  assert.match(css, /input\[type='time'\]::\-webkit-date-and-time-value \{[^}]*min-inline-size: 0;[^}]*text-align: start/s)
...
test('Parcours cards and the weather notice use compact, wrapping visual hierarchies', () => {
  ...
  assert.match(css, /\.weather-detail__state \{[^}]*display: grid;[^}]*grid-template-columns: max-content minmax\(0, 1fr\)/s)
  assert.match(css, /\.weather-detail__notice \{[^}]*grid-column: 1 \/ -1;[^}]*inline-size: 100%;[^}]*padding: 11px;[^}]*border-radius: 9px/s)
  assert.match(css, /\.weather-detail__metrics > \.weather-detail__metric--full \{[^}]*grid-column: 1 \/ -1/s)
  const weatherSource = readFileSync(new URL('../../src/ui/weather-detail.ts', import.meta.url), 'utf8')
  assert.match(weatherSource, /'Conditions les plus défavorables',[\s\S]*'weather-detail__metric--full'/)
})
...
test('bottom navigation reserves content space and Today mobile actions remain shrinkable', () => {
  ...
  assert.match(css, /\.today-weather-points \{[^}]*grid-template-columns: minmax\(0, 1fr\)/s)
  assert.match(css, /@media \(min-width: 700px\) \{[\s\S]*\.today-weather-points \{ grid-template-columns: repeat\(3, minmax\(0, 1fr\)\)/)
  assert.match(css, /\.network-status \{[^}]*position: fixed;[^}]*max-width: calc\(100vw - 24px\)/s)
})
